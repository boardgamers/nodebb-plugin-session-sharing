...
const plugin = {
	...
	settings: {
		name: 'gaiaform',
		cookieName: 'token',
		cookieDomain: 'boardgamers.space',
...
plugin.findOrCreateUser = async (userData) => {
	...
	if (!userId && mergeUid && !isNaN(mergeUid)) {
		winston.info(
			'[session-sharing] Found user via their email, associating this id (' + id + ') with their NodeBB account'
		);
...
plugin.updateUserProfile = async (uid, userData, isNewUser) => {
	...
	if (Object.keys(obj).length) {
		...
		try {
			...
			winston.warn(
				'[session-sharing] Unable to update profile information for uid: ' + uid + '(' + error.message + ')'
			);
		}
	}
...
plugin.addMiddleware = async function ({ req, res }) {
	const { hostWhitelist, guestRedirect, editOverride, loginOverride, registerOverride } = await meta.settings.get(
		'session-sharing'
	);
	...
	function handleGuest(req, res) {
		if (guestRedirect && !req.originalUrl.startsWith(nconf.get('relative_path') + '/login?local=1')) {
			...
			res.redirect(
				guestRedirect.replace('%1', encodeURIComponent(req.protocol + '://' + req.get('host') + req.originalUrl))
			);
	...
	if (editOverride && hasSession && req.originalUrl.match(/\/user\/.*\/edit$/)) {
		return res.redirect(
			editOverride.replace('%1', encodeURIComponent(req.protocol + '://' + req.get('host') + req.originalUrl))
		);
	}
	if (loginOverride && req.originalUrl.match(/\/login$/)) {
		return res.redirect(
			loginOverride.replace('%1', encodeURIComponent(req.protocol + '://' + req.get('host') + req.originalUrl))
		);
	}
	if (registerOverride && req.originalUrl.match(/\/register$/)) {
		return res.redirect(
			registerOverride.replace('%1', encodeURIComponent(req.protocol + '://' + req.get('host') + req.originalUrl))
		);
	}
